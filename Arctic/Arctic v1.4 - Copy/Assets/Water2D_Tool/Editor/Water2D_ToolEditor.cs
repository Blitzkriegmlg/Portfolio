...
namespace Water2DTool
{
    ...
    public class Water2D_ToolEditor : Editor
    {
        ...
        void OnSceneGUI()
        {
            ...
            // Draw and interact with all the 4 handles.
        ...
        private void DrawOverlapSphereGizmo(Water2D_Tool water)
        {
            ...
            int surfaceVertsCount = vertices.Length / 2;
        ...
        private void UpdateHandles(Water2D_Tool water2D, GUIStyle iconStyle)
        {
            ...
            {
                Vector3 pos = water2D.transform.position + Vector3.Scale(new Vector3(water2D.handlesPosition[i].x, water2D.handlesPosition[i].y, 0), water2D.transform.localScale);
                ...
                if (global != pos)
                {
                    ...
                    Vector2 relative = new Vector2(
                        local.x / water2D.transform.localScale.x,
                        local.y / water2D.transform.localScale.y) - water2D.handlesPosition[i];
        ...
        private void DrawRectangle(Water2D_Tool path)
        {
            Handles.color = Color.white;

            for (int i = 2; i < 4; i++)
            {
                Vector3 pos = path.transform.position + path.transform.rotation * Vector3.Scale(new Vector3(path.handlesPosition[i].x, path.handlesPosition[0].y, 0), path.transform.localScale);
                Vector3 pos2 = path.transform.position + path.transform.rotation * Vector3.Scale(new Vector3(path.handlesPosition[i].x, path.handlesPosition[1].y, 0), path.transform.localScale);
                Handles.DrawLine(pos, pos2);
            }

            for (int i = 0; i < 2; i++)
            {
                Vector3 pos = path.transform.position + path.transform.rotation * Vector3.Scale(new Vector3(path.handlesPosition[2].x, path.handlesPosition[i].y, 0), path.transform.localScale);
                Vector3 pos2 = path.transform.position + path.transform.rotation * Vector3.Scale(new Vector3(path.handlesPosition[3].x, path.handlesPosition[i].y, 0), path.transform.localScale);
                Handles.DrawLine(pos, pos2);
            }
        }
