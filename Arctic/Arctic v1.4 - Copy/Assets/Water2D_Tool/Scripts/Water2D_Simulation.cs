...
namespace Water2DTool
{
    ...
    public class Water2D_Simulation : MonoBehaviour
    {
        ...
        /// </summary>
        private List<float> sineAmplitudes;
        /// <summary>
        /// The amount by which a particular sine wave is stretched.
        /// </summary>
        private List<float> sineStretches;
        /// <summary>
        /// Sine wave phase offset.
        /// </summary>
        private List<float> phaseOffset;
        /// <summary>
        ...
        /// </summary>
        private Vector3 waterLineCurrentGlobalPos;
        /// <summary>
        ...
        public Water2D_AnimationMethod animationMethod = Water2D_AnimationMethod.Follow;
        /// <summary>
        ...
        /// </summary>
        public float particleSystemZOffset = 0f;
        /// <summary>
        ...
        void Start()
        {
            ...
            sineAmplitudes = new List<float>();
            sineStretches = new List<float>();
            phaseOffset = new List<float>();
        ...
        void GenerateSineVariables()
        {
            for (int i = 0; i < sineWaves; i++)
            {
                ...
                // The biger the value the faster the waves move to the left (right).
                phaseOffset.Add(Random.Range(minPhaseOffset, maxPhaseOffset));
            }

            if (sineWaves > 1)
            {
                int n = (int)sineWaves / 2;

                // Negative values will make the sine waves move to the left.
                for (int i = 0; i < n; i++)
                {
                    phaseOffset[i] *= -1;
                }
        ...
        void FixedUpdate()
        {
            ...
            {
                if (surfaceWaves == Water2D_SurfaceWaves.SinWaves)
                {
                    SinWaves();
                }

                if (surfaceWaves == Water2D_SurfaceWaves.Random && !makeSplash)
                {
            ...
            if ((a2DCollider ? floatingObjects2D[cIndex].GetComponent<Collider2D>().bounds.min.y : floatingObjects3D[cIndex].GetComponent<Collider>().bounds.min.y) < waterLineCurrentGlobalPos.y)
            {
                ...
                if (a2DCollider)
                {
                    ...
                    if (boxColl2D != null)
                    {
                        ...
                        submergedPolygon = Water2D_PolygonClipping.GetIntersectedPolygon(boXCorners, lineP);
                    ...
                    if (circleColl2D != null)
                    {
                        ...
                        submergedPolygon = Water2D_PolygonClipping.GetIntersectedPolygon(polyCorners, lineP);
                        submergedArea = GetPolygonArea(submergedPolygon);
                    }
                    ...
                    if (polyColl2D != null)
                    {
                        ...
                        submergedPolygon = Water2D_PolygonClipping.GetIntersectedPolygon(polyPoints, lineP);
                        submergedArea = GetPolygonArea(submergedPolygon);
                    }
                }
                else
        ...
        private void GenerateWaterWaves(int cIndex, bool a2DCollider)
        {
            ...
            for (int i = minIndex; i < maxIndex; i++)
            {
                ...
                for (int j = 0; j < len; j++)
                {
                    ...
                    {
                        // A small portion of the object's velocity is added to the vertex.
                        ...
                        if (foundObjOnPrevFrame[i] == false && particleS != null && objectVelocity.y < velocityFilter - 2f)
                        {
                            ...
                            GameObject splash = Instantiate(particleS, new Vector3(vertex1.x, vertex1.y, transform.position.z + water2D.boxColliderZOffset + particleSystemZOffset), Quaternion.Euler(new Vector3(270f, 0, 0))) as GameObject;
                        ...
                        {
                            AudioSource.PlayClipAtPoint(splashSound, vertex1);
        ...
        private void ApplyPhysicsBasedBuoyantForce(int cIndex, bool a2DCollider)
        {
            ...
            if ((a2DCollider ? floatingObjects2D[cIndex].GetComponent<Collider2D>().bounds.min.y : floatingObjects3D[cIndex].GetComponent<Collider>().bounds.min.y) < globalPosOfVert.y)
            {
                ...
                if ((a2DCollider ? floatingObjects2D[cIndex].GetComponent<Collider2D>().tag : floatingObjects3D[cIndex].GetComponent<Collider>().tag) == "Player")
                {
                    ...
                    intersectionPolygon = Water2D_PolygonClipping.GetIntersectedPolygon(boXVertices, linePoints);
                ...
                if (a2DCollider)
                {
                    ...
                    if (circleColl2D != null)
                    {
                        ...
                        intersectionPolygon = Water2D_PolygonClipping.GetIntersectedPolygon(polyCorners, linePoints);
                        ApplyPhysicsForces(cIndex, intersectionPolygon, a2DCollider);
                    }
                    ...
                    if (polyColl2D != null)
                    {
                        ...
                        intersectionPolygon = Water2D_PolygonClipping.GetIntersectedPolygon(polyPoints, linePoints);
                        ApplyPhysicsForces(cIndex, intersectionPolygon, a2DCollider);
                    }
                }
                else
